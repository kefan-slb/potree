...
onmessage = function (event) {
	...
	for (let pointAttribute of pointAttributes.attributes) {
		...
		if (pointAttribute.name === PointAttribute.POSITION_CARTESIAN.name) {
			...
			let buff = new ArrayBuffer(numPoints * 4);
			let values = new Float32Array(buff);

			for (let j = 0; j < numPoints; j++) {
				let value = cv.getFloat32(inOffset + j * pointAttributes.byteSize, true);
				values[j] = value;
			}
			...
			let buff = new ArrayBuffer(numPoints * 4);
			let Is = new Uint32Array(buff);

			for (let j = 0; j < numPoints; j++) {
				let I = cv.getUint32(inOffset + j * pointAttributes.byteSize, true);
				Is[j] = I;
			}
			...
			let buff = new ArrayBuffer(numPoints * 4);
			let Js = new Uint32Array(buff);

			for (let j = 0; j < numPoints; j++) {
				let J = cv.getUint32(inOffset + j * pointAttributes.byteSize, true);
				Js[j] = J;
			}
			...
			let buff = new ArrayBuffer(numPoints * 4);
			let Ks = new Uint32Array(buff);

			for (let j = 0; j < numPoints; j++) {
				let K = cv.getUint32(inOffset + j * pointAttributes.byteSize, true);
				Ks[j] = K;
			}
