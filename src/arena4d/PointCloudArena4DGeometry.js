...
Potree.PointCloudArena4DGeometryNode = class PointCloudArena4DGeometryNode{
	...
	load(){
		...
		xhr.onreadystatechange = function () {
			...
			let size = 29; //17, Kefan: changed

			let buffer = xhr.response;
			let sourceView = new DataView(buffer);
			let numPoints = buffer.byteLength / size;
			let bytesPerPoint = 40; //28, Kefan: changed
			...
			let attributes = [
				...
				Potree.PointAttribute.VALUE,
				Potree.PointAttribute.I,
				Potree.PointAttribute.J,
				Potree.PointAttribute.K,
			];
			...
			let Is = new Int32Array(numPoints);
			let Js = new Int32Array(numPoints);
			let Ks = new Int32Array(numPoints);

			let tightBoundingBox = new THREE.Box3();

			for (let i = 0; i < numPoints; i++) {
				let x = sourceView.getFloat32(i * size + 0, true) + node.boundingBox.min.x;
				let y = sourceView.getFloat32(i * size + 4, true) + node.boundingBox.min.y;
				let z = sourceView.getFloat32(i * size + 8, true) + node.boundingBox.min.z;

				let r = sourceView.getUint8(i * size + 12, true);
				let g = sourceView.getUint8(i * size + 13, true);
				let b = sourceView.getUint8(i * size + 14, true);

				let intensity = sourceView.getUint8(i * size + 15, true);

				let classification = sourceView.getUint8(i * size + 16, true);

				let value = sourceView.getFloat32(i * size + 17, true);
				let I = sourceView.getInt32(i * size + 21, true);
				let J = sourceView.getInt32(i * size + 25, true);
				let K = sourceView.getInt32(i * size + 29, true);
			...
			geometry.addAttribute('I', new THREE.BufferAttribute(Is, 1));
			geometry.addAttribute('J', new THREE.BufferAttribute(Js, 1));
			geometry.addAttribute('K', new THREE.BufferAttribute(Ks, 1));
